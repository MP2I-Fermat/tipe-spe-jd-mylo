...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        \item Si $C$ est un constructeur d'arit\'e $n$ et de param\`etre dans
              ...
              \textit{
                  C'est a dire que on cr\'ee un constructeur qui prend en
...
\begin{example}
    ...
    $(p, e)$ et lorsqu'il n'y a pas d'ambiguit\'e, on \'ecrira $e$ a la place de
...
\begin{remark}
    ...
    \'el\'ements lin\'eaires, \`a ce pr\`es qu'il faut modifier les
    d\'efinitions en consid\'erant que les enfants d'un noeud sont des listes
...
\begin{definition}
    ...
    % Ce ne sont pas des fonctions mathématiques.
    % Par exemple, on pourrait imaginer un contexte ou une premiere lecture
    % depuis stdin renvoie une valeur, et une deuxieme lecture une autre valeur,
    % meme si la meme fonction a ete appellee avec les memes arguments.
    \hl{fonction} de $V$ dans $E$ o\`u $V$ est un ensemble de noms de variables et
...
\begin{remark}
    ...
    n'intervient pas dans l'\'evaluation de cette forme lin\'eaire, ce qui
    justifie r\'etroactivement la notation $e$ pour d\'esigner $[(p, e)]$.
\end{remark}
...
\begin{example}
    ...
    $$
        \lbrack (a_1, \texttt{if} \  v \  \texttt{then} \ l_1 \  \texttt{else} \  l_2),\ v,\ l_1 \rbrack
    $$

    Cependant, son ordre d'\'ex\'ecution dans le contexte
    $[v \rightarrow false]$ est:
    $$
        \lbrack (a_1, \texttt{if} \  v \  \texttt{then} \ l_1 \  \texttt{else} \  l_2),\ v,\ l_2 \rbrack
    $$
\end{example}
...
\begin{definition}
    ...
    $l_i$ apparaît dans l'ordre d'\'execution, alors aucun des
    $(l_j)_{j \in \lBrack 1; n \rBrack}$ n'apparaît apres $l_i$ dans l'ordre
    d'\'ex\'ecution (y compris $l_i$ lui-m\^eme).
\end{definition}
...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        \item Tout \'el\'ement lin\'eaire contenant $e$ (y compris $e$ lui-meme)
              apparaît en dernier dans la forme lin\'eaire qui le contient.
    \end{enumerate}
\end{definition}

\begin{remark}
    Lorsqu'il n'y a pas d'ambiguit\'e, on dira simplement que $e$ est terminal.
\end{remark}
...
\begin{example}
    ...
    On a que :
    \begin{itemize}
        \item $v$ n'est pas localement terminal car il apparaît toujours avant
              l'un de $l_1$ ou $[(b_1, e_1)]$ dans l'ordre d'\'ex\'ecution de
              $\texttt{if}\ v\ \texttt{then}\ l_1\ \texttt{else}\ [(b_1, e_1)]$
        \item $l_1$ et $[(b_1, e_1)]$ sont localement terminaux car $v$ apparaît
              toujours avant eux dans l'ordre d'\'ex\'ecution de
              $\texttt{if}\ v\ \texttt{then}\ l_1\ \texttt{else}\ [(b_1, e_1)]$ et cet
              ordre fait apparaitre au plus un de $l_1$ et $[(b_1, e_1)]$.
...
\begin{definition}
    ...
    \texttt{OCaml} $e$ lorsque Eval($l$, $c$) a le m\^eme r\'esultat (c'est a
    dire renvoie la m\^eme valeur et a les m\^emes effets de bord) que
    Eval\_OCaml($e$, $c$) pour tout contexte $c$.
...
\begin{definition}
    ...
    il existe une forme lin\'eaire correspondant \`a $e$.
\end{definition}
...
\begin{algorithm}
    \caption{Lin\'eariser}
    \Entree{Une expression \texttt{OCaml} $e$ lin\'earisable}
    \Sortie{Une forme lin\'eaire $l$ correcte correspondant \`a $e$}

    \hl{TODO}
\end{algorithm}

Montrons la correction de cet algorithme. Soit alors $e$ une expression
\texttt{OCaml} et $l = \textnormal{Lin\'eariser}(e)$.
