...
\begin{remark}
    ...
    Cependant, nous verrons plus tard que l'on a intérêt à minimiser la taille
    de l'ensemble rectifiant $l$.
\end{remark}
...
\begin{example}
    ...
    \begin{align*}
        g : l \mapsto \textnormal{Eval} \left(
        \begin{aligned}
...
\begin{algorithm}[H]
    \caption{Rectifier}
    \Entree{
        Une forme linéaire correcte $l$, un ensemble de variables
    ...
    \Sortie{
        Une forme linéaire correcte $l'$ telle que $l'$ ne contient aucun
    ...
    fonction de $E$ \\
    $a \leftarrow$ Le motif associé à $e$ dans $l$ \\
    ...
    \uSi{$l_2$ est vide}{
    \uSi{$e$ est un appel à une fonction $f$ de $E$}{
        $e_{rec} \leftarrow$ Un appel à $nouveau\_nom(f)$ avec en arguments les
...
définies par $f_i: x_1, \ldots, x_n \mapsto \textnormal{Eval}(l_i, [x_1 \rightarrow x_1, \ldots, x_n \rightarrow x_n])$
tel que $E$ soit rectifiable.

Construisons un ensemble de fonctions $\{f_1', \ldots, f_n'\}$ par $f_i' : x_1 \rightarrow x_1, \ldots, x_n, \texttt{cont} \mapsto \textnormal{Eval}(\textnormal{Rectifier}(l_i, E, \texttt{cont}, nouveau\_nom), [x_1 \rightarrow x_1, \ldots, x_n \rightarrow x_n, \texttt{cont} \rightarrow \texttt{cont}])$
avec \\ $nouveau\_nom$ défini par $f_i \mapsto f_i'$.
...
On pourra alors redéfinir les $f_i$ par $f_i: x_1, \ldots, x_n \mapsto f_i'\ x_1\ \ldots\ x_n\ \texttt{(fun x -> x)}$
...
\begin{algorithm}[H]
    ...
    \Entree{Un ensemble $\{l_1, \ldots, l_n\}$ de formes linéaires correctes}
    \Sortie{
        Un ensemble $E$ de noms de variables tels que chaque $l_i$ soit
        rectifiable par $E$
    }
...
\begin{remark}
    On considère dans cet algorithme que les noms de variables sont uniques. Si
    deux variables partagent le même nom, on les distinguera, par exemple par
    leur localisation dans le code source. Ceci nous permet bien de parler de
    "la" (et non "les") définitions d'une variable.
\end{remark}

\begin{proposition}
    L'ensemble $E$ renvoyé par Cl\^oture\_rectifiable($\{l_1, \ldots, l_n\}$)
    est tel que chaque $l_i$ est rectifiable par $E$.
\end{proposition}
